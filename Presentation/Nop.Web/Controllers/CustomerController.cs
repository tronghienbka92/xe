...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BasePublicController
    {
        ...
        public ActionResult DangKy(RegisterCustomerModel model)
        {
            ...
            if (model.Phone.Length>6)
                customer.Password = model.Phone.Substring(6);
            else
                customer.Password = model.Phone;
            //mã hóa password
        ...
        [HttpPost]
        public ActionResult TaiKhoan(ChangePasswordModel model)
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return new HttpUnauthorizedResult();
            ...
            if (ModelState.IsValid)
            {
                int intpos = model.Fullname.LastIndexOf(' ');
                string lastname = model.Fullname;
                string firstname = model.Fullname;
                if (intpos > 0)
                {
                    lastname = model.Fullname.Substring(intpos).Trim();
                    firstname = model.Fullname.Substring(0, intpos).Trim();
                }
