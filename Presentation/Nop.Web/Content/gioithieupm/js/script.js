...
$(document).ready(function() {
    ...
    $(".form_submit").click(function() {
        ...
        if (!sodienthoai) {
            $(".form_error .sodienthoai_error").addClass("show").removeClass("hide");
            $("#sodienthoai").focus();
            return false;
        } else {
            $(".form_error .sodienthoai_error").addClass("hide").removeClass("show");
        }
        if (!emaild) {
        ...
        if (name && emaild && message && sodienthoai) {
            buttonclick.html("Đang gửi thông tin...");
            $.ajax({
                url: '/Home/DangKyPhanMem',
                data: {
                    Ten: name,
                    Email: emaild,
                ...
                success: function(data) {
                    $(".Sucess").show();
                    $(".Sucess").fadeIn(2000);
                    $(".Sucess").html("<i class='fa fa-check'></i> Thông tin đăng ký dùng thử của bạn đã được gửi đi, chúng tôi sẽ liên hệ với bạn trong thời gian sớm nhất!");
                    $("#Name").val("");
                    $("#Email").val("");
                    $("#sodienthoai").val("");
                    $("#Message").val("");
                    $(".form_error .name_error, .form_error .email_error, .form_error .email_val_error, .form_error .sodienthoai_error, .form_error .message_error").addClass("hide").removeClass("show");
                    $("#name").val("");
                    $("#email").val("");
                    $("#sodienthoai").val("");
                    $("#diachi").val("");
                    $("#message").val("");
                    buttonclick.html("GỬI ĐĂNG KÝ");
