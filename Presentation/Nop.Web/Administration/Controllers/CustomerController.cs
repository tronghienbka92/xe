...
namespace Nop.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        public ActionResult SendEmailTemplate(CustomerModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCustomers))
                return AccessDeniedView();
            ...
            try
            {
                ...
                if (!CommonHelper.IsValidEmail(customer.Email))
                    throw new NopException("Customer email is not valid");
                if (String.IsNullOrWhiteSpace(model.SendEmail.Subject))
                    throw new NopException("Email template is empty");
                _workflowMessageService.SendCustomerMessage(customer, model.SendEmail.Subject.Trim(), _workContext.WorkingLanguage.Id);
                SuccessNotification(_localizationService.GetResource("Admin.Customers.Customers.SendEmail.Queued"));
